...
class Keyboard:
    ...
    async def account(self, account):
        btns = [[
            self.inl_b('deposit', action=f'get_address {account["symbol"]["id"]}'),
    ...
    async def market_choose_symbol(self, symbols):
        btns = [
            [self.inl_b(symbol['name'].upper(), action=f'market_choose_symbol {symbol["id"]}') for symbol in symbols],
            [self.inl_b('my_orders', action='my_orders')]
        ]
        return InlineKeyboardMarkup(inline_keyboard=btns)
    ...
    async def my_orders(self, orders):
        btns = [
            [
                self.inl_b(
                    f'{"купить" if order["type"] == "buy" else "продать"}, {order["broker"]}, '
        ...
        if orders:
            btns.insert(0,
                [
                    self.inl_b('on_all_orders', action='on_all_orders'),
                    self.inl_b('off_all_orders', action='off_all_orders')
                ]
            )
        return InlineKeyboardMarkup(inline_keyboard=btns)
